...
export const metadata: Metadata = {
  title: 'Public API',
  description: 'API reference for Reposcope public endpoints.',
};
...
export default async function ApiDocsIndexPage() {
  ...
  return (
    ...
    >
      ...
      <DocsDescription>
        {overview?.description ?? 'Reference pages for the public API endpoints exposed by Reposcope.'}
      </DocsDescription>
      <DocsBody>
        <p>
        ...
        <p>
          Reposcope Public APIs (beta) provide a convenient way to access insight data for open source projects on
                    ...
                    >
                      ...
                      <span className="min-w-0 flex-1">
                        <span className="flex items-center gap-2 text-[14px] font-medium text-fd-foreground transition-colors group-hover:text-[#fff2bd]">
                          {item.label}
                          <span className="text-[#7c7c7c] transition-colors group-hover:text-[#22d3ee]">›</span>
                        </span>
        ...
        <h2>Usage</h2>
        <p>
          The Reposcope Public API follows the OpenAPI specification and can be called with tools like <code>curl</code>{' '}
          or any HTTP client library.
        </p>

        <Callout title="Base URL">
          <code>https://api.reposcope.io/v1</code>
        </Callout>
...
x-ratelimit-remaining-minute: 97`}
        ...
        <h3>Example</h3>
        <p>
          To find the countries or regions of stargazers in <code>facebook/react</code>, you can call:
        </p>
        <DynamicCodeBlock
          lang="bash"
          code="curl https://api.reposcope.io/v1/repos/facebook/react/stargazers/countries"
  ...
  "data": {
    ...
    "rows": [
      ...
      { "count": "471", "country_or_area": "JP", "percentage": "0.0304" }
        ...
        <ul>
          <li>Email us at <a href="mailto:hello@reposcope.io">hello@reposcope.io</a>.</li>
          <li>
            Open an issue in the{' '}
            <a href="https://github.com/reposcope/reposcope/issues/new?assignees=&labels=type%2Ffeature&projects=&template=feature_request.md&title=New%20API">
              Reposcope GitHub repository
            </a>.
          </li>
        </ul>
      </DocsBody>
