...
tags: [tidbcloud]
image: /blog-assets/reduce-query-latency/reduce-online-serving-latency-with-distributed-sql-database.jpg
...
The [Reposcope](https://reposcope.io/) website displays the data changes of GitHub events in real time. It's powered by [TiDB Cloud](https://www.pingcap.com/tidb-cloud/), a MySQL-compatible distributed SQL database for elastic scale and real-time analytics.
...
![The repository analysis page was loading](/blog-assets/reduce-query-latency/repository-analysis-page.jpg)
...
For example, after the API server received a request, it executed the following SQL statement to obtain the number of issues in the [vscode repository](https://reposcope.io/analyze/microsoft/vscode):
...
![The query result](/blog-assets/reduce-query-latency/query-result.jpg)
...
![The logical structure](/blog-assets/reduce-query-latency/logical-structure.jpg)
...
In MySQL, a covering index prevents the database from index lookup after index filtering. We wanted to apply this to Reposcope. In our TiDB database, we tried to create a composite index to achieve index coverage.
...
![The result of the EXPLAIN query](/blog-assets/reduce-query-latency/explain-query-result.jpg)
...
![`IndexRangeScan` in the non-clustered table](/blog-assets/reduce-query-latency/indexrangescan-non-clustered.jpg)
...
![`tidb-server` read 270,000 rows of data from `tikv-server`](/blog-assets/reduce-query-latency/read-270000-rows-from-tikv-server.jpg)
...
These composite indexes covered more than 20 analytical queries in repository analysis and personal analysis pages on the Reposcope website. This improved our website's overall loading speed.
