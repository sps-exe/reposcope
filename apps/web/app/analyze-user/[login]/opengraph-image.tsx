...
export default async function Image({ params }: { params: Promise<{ login: string }> }) {
  ...
  const poppinsMedium = fetch(new URL('./SpaceGrotesk.ttf', import.meta.url)).then(r => r.arrayBuffer());
  const poppinsSemiBold = fetch(new URL('./SpaceGrotesk.ttf', import.meta.url)).then(r => r.arrayBuffer());
  ...
  return new ImageResponse(
    (
      <div style={{
        ...
        fontFamily: 'Space Grotesk', position: 'relative', overflow: 'hidden',
      }}>
        ...
        <div style={{ fontSize: 16, fontWeight: 500, color: 'rgba(255,255,255,0.5)', marginTop: 8 }}>Developer Analytics on Reposcope</div>
        ...
        <div style={{ position: 'absolute', bottom: 20, right: 40, fontSize: 18, fontWeight: 500, color: 'rgba(255,255,255,0.4)' }}>reposcope.io</div>
      </div>
    ),
    {
      ...size,
      fonts: [
        { name: 'Space Grotesk', data: await poppinsMedium, style: 'normal' as const, weight: 500 as const },
        { name: 'Space Grotesk', data: await poppinsSemiBold, style: 'normal' as const, weight: 600 as const },
      ],
    },
  );
}
