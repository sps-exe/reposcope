...
export async function generateMetadata ({ params }: { params: Promise<{ login: string }> }): Promise<Metadata> {
  ...
  const description = `Explore ${displayName}'s open source contributions, code activity, and collaboration metrics with Reposcope.`;
  return {
    title,
    description,
    keywords: ['Reposcope', 'developer analytics', 'GitHub', displayName, 'contributions', 'pull requests'],
