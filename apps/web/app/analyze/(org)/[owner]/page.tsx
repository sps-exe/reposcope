...
export async function generateMetadata ({ params }: { params: Promise<{ owner: string }> }): Promise<Metadata> {
  ...
  const fullTitle = `${title} | Reposcope`;
  const description = isOrg
    ? 'Unlock the power of time-flexible organization analytics, community recognition metrics, participant insights, and productivity analysis with Reposcope.'
    : `Explore ${displayName}'s open source contributions, code activity, and collaboration metrics with Reposcope.`;
  return {
    title,
    description,
    keywords: [
