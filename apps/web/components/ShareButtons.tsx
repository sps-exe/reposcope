...
export default function ShareButtons({ url, title, className, stars, forks, language, hashtags }: ShareButtonsProps) {
  ...
  const [origin, setOrigin] = useState('https://reposcope.io');
  ...
  const twitterUrl = `https://twitter.com/intent/tweet?text=${encodeURIComponent(twitterText)}&url=${encodeURIComponent(fullUrl)}&via=Reposcope`;
  ...
  return (
    <div className={`fixed right-3 top-1/2 -translate-y-1/2 z-50 flex flex-col items-center gap-1 rounded-lg bg-[#0b0e14]/80 backdrop-blur p-1.5 shadow-lg ${className ?? ''}`}>
      {/* Native share (mobile) */}
      {canNativeShare && (
        <button
          onClick={nativeShare}
