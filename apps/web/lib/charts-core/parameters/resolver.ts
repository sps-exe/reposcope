import { ParameterDefinitions } from '@/lib/charts-types';
import { API_SERVER, INTERNAL_QUERY_API_SERVER, fetchCollections } from '@/utils/api';
    ...
    switch (def.type) {
      case 'repo-id':
        if (param) {
          ...
          return linkedData.pending.repos[param] = fetch(`${API_SERVER}/gh/repositories/${param}`, { signal })
            .then(handleOApi)
            .then((data) => {
      ...
      case 'user-id':
        if (param) {
          ...
          return linkedData.pending.users[param] = fetch(`${API_SERVER}/gh/user/${param}`, { signal })
            .then(handleOApi)
            .then((data) => {
      ...
      case 'owner-id':
        if (param) {
          ...
          return linkedData.pending.orgs[param] = fetch(`${API_SERVER}/gh/user/${param}`, { signal })
            .then(handleOApi)
            .then((data) => {
      ...
      case 'collection-id':
        if (param) {
          ...
          return linkedData.pending.collections[param] = collectionsPromise
            .then((res) => res.find((collection) => String(collection.id) === String(param)))
            .then((collection) => {
              if (collection) {
                return linkedData.collections[param] = {
                  id: collection.id,
                  name: collection.name,
                  public: Boolean(collection.public),
                };
              }
              return linkedData.collections[param] = { id: parseInt(param as any), public: false, name: 'Unknown' };
            })
            .finally(() => {
      ...
      case 'repo-ids':
        if (originParam) {
          ...
          return (linkedData.pending.repos[originParamListStr] = fetch(
            `${INTERNAL_QUERY_API_SERVER}/repos?${urlSearchParams.toString()}`,
            { signal }
          )
            .then(handleOApi)
            .then(
