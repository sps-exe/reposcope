...
import { ComposeVisualizationConfig, VisualizerModule } from '@/lib/charts-types';
    ...
    if (size.width === 0 || size.height === 0) {
      ...
      Promise
        .all(items.map(item => {
          ...
          const loader = (visualizer as any).subVisualizers?.[item.widget];
          if (!loader) {
            console.warn(`[ComposeComponent] No sub-visualizer for: ${item.widget}`);
            return undefined;
          }
          return loader();
        }))
  ...
  return (
    ...
    >
      {config.map(({ parameters, widget, data, ...props }, i) => {
        // Sanitize NaN values in layout props to avoid invalid CSS
        const safeProps = Object.fromEntries(
          Object.entries(props).map(([k, v]) => [k, typeof v === 'number' && isNaN(v) ? 0 : v])
        );
        if (widget.startsWith('builtin:')) {
          return <Builtin key={i} className="absolute" style={safeProps} name={widget as any} colorScheme={colorScheme}  {...parameters} />;
        } else {
          if (loading) {
            return undefined;
          }
