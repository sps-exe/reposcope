...
describe('get user by id', () => {
  ...
  test('user can be found by id', async () => {
    const [rs] = await conn.query<ResultSetHeader>(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?);
    `, ['reposcope', 'reposcope@pingcap.com', 1, 'https://github.com/reposcope.png', UserRole.USER, '2022-01-01 00:00:00', 1]);
    ...
    await conn.query<ResultSetHeader>(`
      ...
      VALUES (?, ?, ?, ?, ?);
    `, [userId, ProviderType.GITHUB, 1001, 'reposcope', 'token']);

    const userProfile = await userService.getUserById(1);
    expect(userProfile).toEqual({
      id: userId,
      name: 'reposcope',
      emailAddress: 'reposcope@pingcap.com',
      emailGetUpdates: true,
      githubId: 1001,
      githubLogin: 'reposcope',
      avatarURL: 'https://github.com/reposcope.png',
      role: UserRole.USER,
...
describe('find or create user by account', () => {
  ...
  beforeEach(async () => {
    ...
    await conn.query(
      ...
      [1, 'foo', 'test@example.com', 'https://github.com/reposcope.png', 'user']
    );
    await conn.query(`SET FOREIGN_KEY_CHECKS = 1;`);
  });
  ...
  test('create a new user to bound to the account', async () => {
    ...
    .mockImplementation(
      async (domain: string, token: string): Promise<Auth0UserInfo> => {
        return {
          ...
          nickname: "reposcope",
          name: "reposcope",
          picture: "https://github.com/reposcope.png",
          updated_at: new Date().toUTCString(),
          email: "reposcope@pingcap.com",
          email_verified: true,
        };
      }
    );
    ...
    const account = {
      ...
      providerAccountLogin: 'reposcope',
      accessToken: 'token'
    };
