...
export const AUTH0_NAMESPACE = `https://reposcope.io`;
...
export interface Auth0User {
    "https://reposcope.io/metadata": Auth0UserMetadata;
    iss: string;
    sub: string;
    aud: string[];
    iat: number;
...
export function parseAuth0User(user: Auth0User) {
    const { ["https://reposcope.io/metadata"]: metadata, ...others } = user;
    return {
        ...others,
        metadata,
    };
