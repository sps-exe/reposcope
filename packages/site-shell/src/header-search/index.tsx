...
export function HeaderAnalyzeSelector({ apiBase = '', navigateTo }: HeaderAnalyzeSelectorProps) {
  ...
  return (
    <>
      ...
      >
        <LucideSearchIcon className="size-4 text-[#22d3ee]" />
        Search ...
      ...
      <Dialog open={isOpen} onOpenChange={(open) => { if (!open) closeModal(); }}>
        <DialogContent
          ...
          className="search-dialog top-[10vh] max-h-[80vh] translate-y-0 overflow-y-auto rounded-sm border border-white/10 bg-[#0b0e14] p-0 text-[#e9eaee] shadow-[0_40px_120px_-60px_rgba(0,0,0,0.98)] ring-1 ring-black/20 supports-backdrop-filter:bg-[#0b0e14]/98 supports-backdrop-filter:backdrop-blur-2xl"
        >
          <DialogTitle className="sr-only">Search</DialogTitle>
          <DialogDescription className="sr-only">
            Search developers, repositories, and organizations to jump into Reposcope analysis pages.
          </DialogDescription>
      ...
      {types.map((tab) => (
        <Button
          ...
          className={twMerge(
            ...
            tab.id === selectedType ? 'bg-[#12151f] text-[#22d3ee] hover:bg-[#2a2a2c]' : 'text-slate-300 hover:bg-white/[0.05] hover:text-slate-100',
          )}
...
function BottomTips() {
  return (
    <div className="mt-auto flex cursor-default select-none flex-wrap items-center gap-3 border-t border-white/6 bg-[#12151f] px-6 py-3 text-sm text-[#a8aab8]">
  ...
  return (
    <div className="relative">
      ...
      <Input
        ref={inputRef}
        type="text"
        className="h-11 rounded-md border-white/10 bg-[#12151f] pl-9 text-[#eef0f5] placeholder:text-[#84879a] focus-visible:border-[#22d3ee]/45 focus-visible:ring-2 focus-visible:ring-[#22d3ee]/18"
